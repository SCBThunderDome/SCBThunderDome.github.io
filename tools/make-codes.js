...
const LEAGUE_BY_DIGIT = {
  1: { slug: "scbthunderdome", label: "SCB Thunderdome" },
};
...
function parseLeagues(input) {
  const digits = String(input).match(/[1]/g);
  if (!digits) return null;
  /* Dedupe. One league today; the loop is kept so adding a second
     league is a line in LEAGUE_BY_DIGIT and a digit here. */
  const slugs = [];
  for (const d of ["1"]) {
...
`);
  ...
  https://scbthunderdome.github.io/admin/ — nothing to install.
